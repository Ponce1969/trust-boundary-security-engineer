...
# Domain-Specific Sufficiency Instantiation

While this core principle establishes that a control must be sufficient against the system's actual execution model and threat surface, specialized domains instantiate containment across domain-specific boundaries according to `specs/SPEC-006-skill-extension-protocol.md`.

For example, specialized domain skills (such as `agentic-security-auditor`) map control sufficiency across multiple independent containment boundaries (e.g., semantic instruction boundaries, architectural sandbox isolation, and API least privilege). A control at one layer is only declared sufficient if an upstream failure is demonstrably contained by downstream structural boundaries. → `specs/SPEC-006-skill-extension-protocol.md`, `knowledge/defense-in-depth.md`
